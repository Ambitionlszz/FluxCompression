...
class AnalysisTransform(nn.Module):
    """
    g_a: Analysis Transform with 4x downsampling.
    Input: (B, C_in, H, W) -> Output: (B, channel, H/4, W/4)
    """

    def __init__(self, ch_emd=128, channel=320):
        super().__init__()
        # Align spatial dimensions: both paths should downsample by 2x to match
        self.pre1 = nn.Sequential(
            nn.Conv2d(ch_emd, 128,kernel_size = 3,padding=1), # e.g., 128ch -> 128ch, 32x32 -> 16x16 (for FLUX.2-klein-4B)
        )
        ...
        self.analysis_transform = nn.Sequential(
            ...
            DepthConvBlock(192, 192),
            Downsample(192, 320),  # 1st downsample (2x) -> 16x16 -> 8x8
            DepthConvBlock(320, 320),
            Downsample(320, channel),
...
class SynthesisTransform(nn.Module):
    """
    g_s: Synthesis Transform with 4x upsampling.
    Input: (B, channel, H, W) -> Output: (B, channel_out, H*4, W*4)
    """

    def __init__(self, channel=320, channel_out=32) -> None:
        super().__init__()
        # Modified for 4x upsampling using Upsample (PixelShuffle) as per DiT-IC design
        self.synthesis_transform = nn.Sequential(
            DepthConvBlock(channel, 320),
            DepthConvBlock(320, 320),
            Upsample(320, 320),      # 1st upsample (2x) -> 4x4 -> 8x8
            DepthConvBlock(320, 320),
            Upsample(320, 320),      # 2nd upsample (2x) -> 8x8 -> 16x16
...
class SynthesisTransform2(nn.Module):
    """Scale prediction with 4x upsampling."""
    ...
    def __init__(self, channel=320, channel_out=32) -> None:
        ...
        self.synthesis_transform = nn.Sequential(
            DepthConvBlock(channel, 320),
            DepthConvBlock(320, 320),
            Upsample(320, 192),      # 1st upsample (2x)
            DepthConvBlock(192, 192),
...
class AuxDecoder(nn.Module):
    ...
    def __init__(self, ch_emd=32, channel=320) -> None:
        super().__init__()
        # Modified for 4x upsampling to match g_s output size
        self.block = nn.Sequential(
            DepthConvBlock(channel, 320),
            DepthConvBlock(320, 320),
            Upsample(320, 320),      # 1st upsample (2x)
            DepthConvBlock(320, 320),
...
class HyperAnalysis(nn.Module):
    def __init__(self, channel=320) -> None:
        super().__init__()
        # h_a uses ResidualBlockWithStride2 as per DiT-IC original design
...
class HyperSynthesis(nn.Module):
    def __init__(self, channel=320) -> None:
        super().__init__()
        # h_s uses ResidualBlockUpsample2 as per DiT-IC original design
...
class LatentCodec(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # Removed self.prompt as we use Qwen text encoder directly in Flux2 pipeline
        ...
        self.adapter_out = nn.ModuleList(
            Adapter(in_ch=context_dim, out_ch=M * 2) for i in range(4)
        )
    ...
    def sequeeze_with_mask(self, latent, mask):
        ...
        latent_sequeeze = (
            latent_group_1 * mask_group_1
            + latent_group_2 * mask_group_2
            + latent_group_3 * mask_group_3
            + latent_group_4 * mask_group_4
        )
    ...
    def unsequeeze_with_mask(self, latent_sequeeze, mask):
        ...
        latent = torch.cat(
            (
                latent_sequeeze * mask_group_1,
                latent_sequeeze * mask_group_2,
                latent_sequeeze * mask_group_3,
                latent_sequeeze * mask_group_4,
            ),
            dim=1,
        )
        return latent

    def compress_group_with_mask(
        self, gaussian_conditional, latent, scales, means, mask, symbols_list, indexes_list
    ):
        ...
        latent_squeeze_hat = gaussian_conditional.quantize(
            latent_squeeze, "symbols", means_squeeze
        )
    ...
    def decompress_group_with_mask(
        self, gaussian_conditional, scales, means, mask, decoder, cdf, cdf_lengths, offsets
    ):
        scales_squeeze = self.sequeeze_with_mask(scales, mask)
        means_squeeze = self.sequeeze_with_mask(means, mask)
        indexes = gaussian_conditional.build_indexes(scales_squeeze)
        latent_squeeze_hat = decoder.decode_stream(
            indexes.reshape(-1).tolist(), cdf, cdf_lengths, offsets
        )
        latent_squeeze_hat = (
            torch.Tensor(latent_squeeze_hat).reshape(scales_squeeze.shape).to(scales.device)
        )
        latent_hat = self.unsequeeze_with_mask(latent_squeeze_hat + means_squeeze, mask)
        return latent_hat
    ...
    def forward_with_mask(self, latent, scales, means, mask):
        ...
        _, y_likelihoods = self.gaussian_conditional(
            latent_squeeze, scales_squeeze, means=means_squeeze
        )
    ...
    def forward(self, latent, latent2):
        ...
        base = self.h_s(z_hat)
        means_0_supp, scales_0_supp = self.adapter_out[0](
            self.g_c(self.adapter_in[0](base))
        ).chunk(2, 1)
        ...
        means_1_supp, scales_1_supp = self.adapter_out[1](
            self.g_c(self.adapter_in[1](base))
        ).chunk(2, 1)
        ...
        means_2_supp, scales_2_supp = self.adapter_out[2](
            self.g_c(self.adapter_in[2](base))
        ).chunk(2, 1)
        ...
        means_3_supp, scales_3_supp = self.adapter_out[3](
            self.g_c(self.adapter_in[3](base))
        ).chunk(2, 1)
        ...
        latent_scales = self.scale_with_mask(
            scales_0_supp, scales_1_supp, scales_2_supp, scales_3_supp, mask_0, mask_1, mask_2, mask_3
        )
        mean = self.g_s(y_hat)
        ...
        # 返回字典（与 Flow 项目的 TCMLatent 保持一致）
        return {
            "x_hat": mean,  # 重建的 latent（均值）
            "scale": scale,
            "res": res,  # 辅助分支输出
    ...
    def compress(self, latent, latent2):
        ...
        y_hat_0 = self.compress_group_with_mask(
            self.gaussian_conditional, y, scales_0_supp, means_0_supp, mask_0, symbols_list, indexes_list
        )
        lrp = self.LRP[0](torch.cat([y_hat_0, base], dim=1)) * mask_0
        lrp = 0.5 * torch.tanh(lrp)
        y_hat_0 = y_hat_0 + lrp
        ...
        y_hat_1 = self.compress_group_with_mask(
            self.gaussian_conditional, y, scales_1_supp, means_1_supp, mask_1, symbols_list, indexes_list
        )
        lrp = self.LRP[1](torch.cat([y_hat_1, base], dim=1)) * mask_1
        lrp = 0.5 * torch.tanh(lrp)
        y_hat_1 = y_hat_1 + lrp
        ...
        y_hat_2 = self.compress_group_with_mask(
            self.gaussian_conditional, y, scales_2_supp, means_2_supp, mask_2, symbols_list, indexes_list
        )
        lrp = self.LRP[2](torch.cat([y_hat_2, base], dim=1)) * mask_2
        lrp = 0.5 * torch.tanh(lrp)
        y_hat_2 = y_hat_2 + lrp
        ...
        y_hat_3 = self.compress_group_with_mask(
            self.gaussian_conditional, y, scales_3_supp, means_3_supp, mask_3, symbols_list, indexes_list
        )
        lrp = self.LRP[3](torch.cat([y_hat_3, base], dim=1)) * mask_3
        lrp = 0.5 * torch.tanh(lrp)
        y_hat_3 = y_hat_3 + lrp
    ...
    def decompress(self, strings, shape):
        ...
        # Note: In decompress, the spatial dimensions of y are 4x larger than z
        B, C, H, W = z_hat.shape
        ...
        y_hat_0 = self.decompress_group_with_mask(
            self.gaussian_conditional, scales_0_supp, means_0_supp, mask_0, decoder, cdf, cdf_lengths, offsets
        )
        lrp = self.LRP[0](torch.cat([y_hat_0, base], dim=1)) * mask_0
        lrp = 0.5 * torch.tanh(lrp)
        y_hat_0 = y_hat_0 + lrp
        ...
        y_hat_1 = self.decompress_group_with_mask(
            self.gaussian_conditional, scales_1_supp, means_1_supp, mask_1, decoder, cdf, cdf_lengths, offsets
        )
        lrp = self.LRP[1](torch.cat([y_hat_1, base], dim=1)) * mask_1
        lrp = 0.5 * torch.tanh(lrp)
        y_hat_1 = y_hat_1 + lrp
        ...
        y_hat_2 = self.decompress_group_with_mask(
            self.gaussian_conditional, scales_2_supp, means_2_supp, mask_2, decoder, cdf, cdf_lengths, offsets
        )
        lrp = self.LRP[2](torch.cat([y_hat_2, base], dim=1)) * mask_2
        lrp = 0.5 * torch.tanh(lrp)
        y_hat_2 = y_hat_2 + lrp
        ...
        y_hat_3 = self.decompress_group_with_mask(
            self.gaussian_conditional, scales_3_supp, means_3_supp, mask_3, decoder, cdf, cdf_lengths, offsets
        )
        lrp = self.LRP[3](torch.cat([y_hat_3, base], dim=1)) * mask_3
        lrp = 0.5 * torch.tanh(lrp)
        y_hat_3 = y_hat_3 + lrp
        ...
        # 返回字典（与 forward 保持一致，注意 prompt 已在当前版本中移除）
        return {
            "x_hat": mean,
            "scale": scale,
            "res": res,
...
if __name__ == "__main__":
    model = LatentCodec()
    x = torch.randn(1, 32, 32, 32)  # Main latent (16ch scaled to 32 for testing)
    y = torch.randn(1, 320, 32, 32)  # Aux latent
    out = model(x, y)
    print("Mean shape:", out["mean"].shape)
    print("Scale shape:", out["scale"].shape)
    print("Res shape:", out["res"].shape)
