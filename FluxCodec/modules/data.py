    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        ...
        while attempts < max_attempts:
            try:
                ...
                print(f"\n[Dataset Warning] Skipping corrupted image '{self.samples[idx]}' due to: {e}")
                idx = random.randint(0, len(self.samples) - 1)
                attempts += 1
        raise RuntimeError("Too many corrupted images in the dataset, cannot load any valid image.")
