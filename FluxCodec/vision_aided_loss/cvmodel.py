...
class DINOv2Reg(torch.nn.Module):
    ...
    def __init__(self, cv_type='adv', repo_or_dir=None, weights=None):
        super().__init__()

        self.cv_type = cv_type
        repo_or_dir = repo_or_dir or os.environ.get("DINOV2_REPO")
        weights = weights or os.environ.get("DINOV2_WEIGHTS")
        hub_kwargs = {}
        if weights:
            hub_kwargs["weights"] = weights
        if repo_or_dir:
            self.model = torch.hub.load(repo_or_dir, 'dinov2_vitb14_reg', source='local', **hub_kwargs)
        else:
            self.model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14_reg', **hub_kwargs)
        self.model.eval()
        self.model.requires_grad = False
        self.input_resolution = 224
        self.image_mean = torch.tensor([0.485, 0.456, 0.406])
...
class CVBackbone(torch.nn.Module):
    ...
    def __init__(self, cv_type, output_type, diffaug=False, device='cpu', dinov2_repo=None, dinov2_weights=None):
        super().__init__(
        )
        cv_type = cv_type.split('+')
        output_type = output_type.split('+')
        self.class_name_dict = {
                'seg_ade': 'FluxCodec.vision_aided_loss.swintaskspecific.Swin',
                'det_coco': 'FluxCodec.vision_aided_loss.swintaskspecific.Swin',
                'clip': 'FluxCodec.vision_aided_loss.cvmodel.CLIP',
                'dino': 'FluxCodec.vision_aided_loss.cvmodel.DINO',
                'vgg': 'FluxCodec.vision_aided_loss.cvmodel.Vgg',
                'swin': 'FluxCodec.vision_aided_loss.cvmodel.Swin',
                'face_seg': 'FluxCodec.vision_aided_loss.face_parsing.Parsing',
                'face_normals': 'FluxCodec.vision_aided_loss.face_normals.Normals'
            }
        ...
        for cv_type_, output_type_ in zip(cv_type, output_type):
            ...
            if cv_type_ == 'dino':
                ...
                model = lambda cv_type: DINOv2Reg(
                    cv_type=cv_type,
