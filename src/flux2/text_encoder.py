...
class Qwen3Embedder(nn.Module):
    def __init__(
        ...
        tokenizer_path: str = None,  # 新增参数：tokenizer 路径
    ):
        super().__init__()

        # 如果未指定 tokenizer_path，则使用 model_spec（保持向后兼容）
        if tokenizer_path is None:
            tokenizer_path = model_spec

        # 如果提供了量化配置，使用它来减少显存占用
        if quantization_config is not None:
            print(f"Loading Qwen3 with quantization (8-bit) to save memory...")
