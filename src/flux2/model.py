    ...
    def forward(
        ...
        for block in self.double_blocks:
            if getattr(self, "gradient_checkpointing", False):
                img, txt = torch_checkpoint.checkpoint(
                    block, img, txt, pe_x, pe_ctx, double_block_mod_img, double_block_mod_txt, use_reentrant=False
                )
            else:
                img, txt = block(
                    img,
        ...
        for i, block in enumerate(self.single_blocks):
            if getattr(self, "gradient_checkpointing", False):
                img = torch_checkpoint.checkpoint(
                    block, img, pe, single_block_mod, use_reentrant=False
                )
            else:
                img = block(
                    img,
